...
import { Question, QuestionData } from '../../interfaces/question';
...
export class QuizHomeComponent implements OnInit {
	...
	private loadQuestions(): void {
		...
		this.subscriptions.add(
			this.quizStore.pipe(select(selectQuestionsLoading)).subscribe((isLoading) => {
	...
	private loadQuizResponse(): void {
		...
		this.subscriptions.add(
			this.quizStore.pipe(select(selectQuizResponseLoading)).subscribe((isLoading) => {
				this.isLoading = isLoading;
				this.cdr.markForCheck();
			})
		);
		this.subscriptions.add(
			this.quizStore.pipe(select(selectQuizResponseError)).subscribe((error) => {
				this.error = error;
				this.cdr.markForCheck();
			})
		);
	...
	public onNextClick(data: QuestionData): void {
	...
	public onPrevClick(data: QuestionData): void {
